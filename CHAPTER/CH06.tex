...
Grade 4.6 bolts are made of low carbon steel (similar to Grade 250 steel) and they are used mostly in secondary members.

Grade 8.8 bolts are made of medium carbon steel using quenching and tempering to enhance the properties. They are used in main framing.
...
\begin{figure}[H]
\centering\input{PIC/CH06/IW}\caption{Definition of impact wrench sizes \citep{ASI2016}}
\end{figure}
...
Proof loading a bolt involves tightening the bolt such that it is subject to a minimum tension, known as the `bolt proof load', $N_\text{proof}$.
The bolt in tension, applies a compression force of $N_\text{proof}$ to the plates.
The compression is a `normal force' over the interface between the plates.
This minimises axial deformation due to tension over the plate interfaces, and slip deformation when shear is applied over the interface.

The minimum proof load for specific bolts is specified in the following table (\NZSSTEEL{Table~15.2.5.1}).
\begin{table}[H]\centering\footnotesize\caption{Proof load $N_\text{proof}$ for property class 8.8 bolts}\label{tab:proof_load}
\begin{tabular}{cc}
	\toprule
	Nominal diameter of bolt & Proof load ($N_\text{proof}$, \si{\kn}) \\ \midrule
	          M16            &                     95                      \\
	          M20            &                     145                     \\
	          M22            &                     180                     \\
	          M24            &                     210                     \\
...
\begin{tabular}{c|ccc}
	...
	Size &           Shear           &           Shear           &          Axial           \\
	     ...
	     & $\phi{}V_{fx}~(\si{\kn})$ & $\phi{}V_{fn}~(\si{\kn})$ & $\phi{}N_{tf}~(\si{\kn})$ \\ \midrule
	M12  &           22.4            &           15.1            &            27            \\
	M16  &           39.9            &           28.6            &           50.2           \\
	M20  &           62.3            &           44.6            &           78.4           \\
	M24  &           89.7            &           64.3            &           113            \\
...
\begin{tabular}{c|ccc|cc|ccc}
	...
	Size & \multicolumn{3}{l|}{Strength /S or /T}                                                 & \multicolumn{5}{l}{Serviceability /T}                                                                                      \\ \cline{2-9}
	     ...
	     & $\phi{}V_{fx}$ ($\si{\kn}$) & $\phi{}V_{fn}$ ($\si{\kn}$) & $\phi{}N_{tf}$ ($\si{\kn}$) & $N_\text{proof}$ ($\si{\kn}$) & $\phi{}N_\text{proof}$ ($\si{\kn}$) & $k_h=1.0$  & $k_h=0.85$ &                   $k_h=0.7$                    \\ \midrule
...
\end{conditions}
\begin{figure}[H]
\centering\input{PIC/CH06/ED1}\caption{Illustration of $a_e$}
\end{figure}
\end{enumerate}

The following requirements shall be met to avoid potential failure modes.
\begin{figure}[H]
\centering\input{PIC/CH06/ED2}\caption{Illustration of minimum pitch $s$ and edge distance $e$}
\end{figure}
\begin{enumerate}
...
\NZSSTEEL{\S~9.6.4} requires the maximum distance from the centre of any bolt hole to the edge of the plate to be the smaller of $12t_p$ and $\SI{150}{\mm}$.
\begin{gather}
...
\end{gather}
\begin{figure}[H]
\centering\input{PIC/CH06/VI}\caption{Envelope of axial force versus shear}
\end{figure}
\subsection{Bending Failure of Bolts}
Bending failure is not usually a problem except for very long bolts. Standard methods can be used.
\begin{figure}[H]
\centering
...
Try 75\texttimes50\texttimes5UA, $A_g=\SI{560}{\mm^2}>\SI{555.6}{\mm^2}$, OK. Check net area,
...
\end{conditions}
The minimum bolt tension at installation, $N_{ti}$, may be taken as $N_\text{proof}$ for proof loaded bolts (\tabref{tab:proof_load}).
\begin{figure}[H]
\centering
\input{PIC/CH06/SS}\caption{Friction force}
\end{figure}
\subsection{Gap Opening (Tension Behaviour)}
\begin{gather}
N^*_{tf}\leqslant\phi{}N_{ti},
\end{gather}
where
\begin{conditions}
\phi&strength reduction factor, \num{0.7}\\
N_{ti}&minimum bolt tension at installation
\end{conditions}
The minimum bolt tension at installation, $N_{ti}$, may be taken as $N_\text{proof}$ for proof loaded bolts.
\subsection{Shear Slip Under Tension}\label{sec:bolt_combined_s}
In combined shear and tension, a bolt shall satisfy
\begin{gather}
\dfrac{V^*_{sf}}{\phi{}V_{sf}}+\dfrac{N^*_{tf}}{\phi{}N_{ti}}\leqslant1.0.
\end{gather}
The minimum bolt tension at installation, $N_{ti}$, may be taken as $N_\text{proof}$ for proof loaded bolts.
\begin{figure}[H]
\centering\input{PIC/CH06/VN}\caption{Envelope of tension versus shear}
\end{figure}
