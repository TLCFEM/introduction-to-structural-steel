...
\node(exp.presentation)[right=4cm of presentation.center]{clear construction drawings (draftsperson)};
\node(exp.approval)[right=4cm of approval.center]{verification with client that the structure satisfies needs};
\node(exp.production)[right=4cm of production.center]{production of final production documents};
\node(exp.contracting)[right=4cm of contracting.center]{letting contract};
\node(exp.overseeing)[right=4cm of overseeing.center]{checking that structure is build according to specification and solving site problems};
\end{scope}
\end{tikzpicture}