...
\draw[0066cc,line width=.8mm](-3,-2)--(D);
...
\draw[|<->|]($(-3,-2)-(0,.5)$)--($(A)-(0,2.5)$)node[midway,fill=white]{$b_{0}$};
\draw[|<->|]($(A)-(0,2.5)$)--($(B)-(0,2.5)$)node[midway,fill=white]{$b_{bf}$};
\draw[|<->|]($(B)-(0,2.5)$)--($(D)-(0,.5)$)node[midway,fill=white]{$b_{bw}$};
\draw[|<->|]($(-3,-2)-(0,1)$)--($(D)-(0,1)$)node[midway,fill=white]{$b_{b}=b_0+b_{bf}+b_{bw}$};
\draw[|<->|](-4.5,-2)--++(0,2)node[midway,fill=white]{$\dfrac{d_1}{2}$};
\draw(4,-2)pic{ext};
\draw[<-](-3,-3.5)--++(-.5,-.5)--++(-1,0)node[fill=white,align=center]{beam\\end};
\end{tikzpicture}